...
function Login({ onLoginSuccess, onBack }) {
  ...
  // EXACT SAME DSSN CHALLENGE FUNCTION - Updated service name to Agriculture
  const requestDSSNChallenge = async (dssn) => {
    try {
      ...
      const response = await api.post('/gov-services/request', { 
        dssn, 
        service: "Digital Liberia Agriculture System", // Changed to Agriculture System
        fcmToken,
        requestData: {
          timestamp: new Date().toISOString(),
          service: "Digital Liberia Agriculture - Farmer Access", // Updated service name
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log('Initiating agriculture DSSN challenge for:', dssn);
      ...
      const interval = setInterval(async () => {
        try {
          ...
          if (statusResponse.status === 'approved') {
            ...
            setPolling(false);
            console.log('Agriculture system login approved with token:', statusResponse.govToken);
            ...
            clearInterval(interval);
            setPolling(false);
            setError("Agriculture access was denied on your mobile device");
          }
        } catch (error) {
          console.error('Error polling challenge status:', error);
          clearInterval(interval);
      ...
      setTimeout(() => {
        if (polling) {
          clearInterval(interval);
          setPolling(false);
          setError("Agriculture verification timed out. Please try again.");
        }
      }, 5 * 60 * 1000);
  ...
  return (
    <div className="login-container">
      <div className="login-content">
        ...
        >
          ← Back to AgriTech Portal
        </button>

        <div className="login-header">
          <div className="login-logo">🌱</div>
          <h2>Agriculture Professional Verification</h2>
          <p>
            Enter your DSSN to access Digital Liberia Agriculture System
          </p>
          <div style={{
            ...
            🌾 Secure agriculture access for authorized professionals only
          </div>
        </div>
        ...
        <div className="login-form">
          ...
          {polling ? (
            <div className="verification-pending">
              ...
              <h3 style={{ margin: '1.5rem 0', color: 'var(--text-dark)', fontSize: '1.5rem' }}>
                Verifying Agriculture Access
              </h3>
              <p style={{ color: 'var(--text-light)', marginBottom: '1.5rem', fontSize: '1.1rem' }}>
                Please check your mobile app to approve this agriculture access request.
              </p>
              ...
              <p className="challenge-id">
                Agriculture Verification ID: {challengeId}
              </p>
              <p className="timeout-notice">
                This agriculture access request will timeout in 5 minutes
              </p>
            </div>
          ) : (
            <form onSubmit={handleSubmit}>
              <div className="form-group">
                ...
                <p className="input-help">
                  Enter your DSSN and approve the agriculture access request on your mobile app
                </p>
              </div>
          ...
          <div className="login-footer">
            <p className="access-info" style={{ 
              ...
              🌱 Secure agriculture system for authorized professionals
            </p>
          </div>
        </div>
      </div>
